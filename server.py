...
TOOLS = [
    ...
    # ───────────── НОВОЕ: КОММЕНТАРИИ К ЗАДАЧАМ ─────────────
    {
        "name": "tasks_get_comments",
        "description": "Получить комментарии к задаче по её ID",
        "inputSchema": {
            "type": "object",
            "properties": {
                "task_id": {"type": "integer", "description": "ID задачи"}
...
async def call_tool(name: str, args: dict) -> str:
    try:
        ...
        # ───────────── НОВОЕ: КОММЕНТАРИИ К ЗАДАЧАМ ─────────────
            ...
            r = await b24("task.comment.getlist", {"TASK_ID": args["task_id"]})
            return json.dumps(r.get("result", []), ensure_ascii=False, indent=2)

        elif name == "tasks_add_comment":
            r = await b24("task.comment.add", {
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "bitrix24-mcp", "version": "2.2.0"}
            }
        }
